...
from commands.db import getperevod, getperevodbtc, getidname, getlimitdb, getlimitbtcdb, getstatus, url_name, get_balance, get_btc
...
async def bdat_cmd(message: types.Message):
    ...
    try:
        ...
        btc_amount = int(float(btc_amount))
        btc_amount2 = '{:,}'.format(btc_amount).replace(',', '.')
    except:
...
$
💫 Вы уже передали: {perbtc2}B
🚀 У вас осталось: {ost}$ для передачи!
🚀 У вас осталось: {op}$ для передачи''', parse_mode='html')
